...
public class EmployeeServiceUnitTest {
    ...
    public void testResetPassword_Success() {
        ...
        when(userServiceClient.setEmployeePassword(resetPasswordDTO)).thenReturn(successResponse);

        // Izvršavanje metode koju testiramo
        employeeService.resetPassword(tryPasswordResetDTO);

        verify(employeeActivationRepository, times(1)).findEmployeeActivationByIdentifierAndActivationPossibleIsTrue(tryPasswordResetDTO.getIdentifier());
    }
    ...
    public void testResetPassword_Failure() {
        ...
        SetPasswordDto resetPasswordDTO = new SetPasswordDto(/* Popunite polja za ResetPasswordDTO */);
        ResponseEntity<String> errorResponse = new ResponseEntity<>("Error message", HttpStatus.BAD_REQUEST);

        when(employeeActivationRepository.findEmployeeActivationByIdentifierAndActivationPossibleIsTrue(tryPasswordResetDTO.getIdentifier())).thenReturn(Optional.of(employeeActivation));
        when(userServiceClient.setEmployeePassword(resetPasswordDTO)).thenReturn(errorResponse);
